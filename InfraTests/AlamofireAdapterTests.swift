...
class AlamofireAdapterTests: XCTestCase {
    ...
    func teste_post_should_complete_with_error_on_invalid_cases() {
    ...
    func teste_post_should_complete_with_no_data_when_request_completes_with_200() {
    ...
    func teste_post_should_complete_with_data_when_request_completes_with_204() {
        expectResult(.success(makeValidData()), when: (data: makeValidData(), response: makeHttpResponse(), error: nil))
    }
    func teste_post_should_complete_with_error_when_request_completes_with_non_200() {
        ...
        expectResult(.failure(.anauthorized), when: (data: makeValidData(), response: makeHttpResponse(statusCode: 401), error: nil))
...
extension AlamofireAdapterTests {
    ...
    func expectResult(_ expectedResult: Result<Data?, HttpError>, when stub: (data: Data?, response: HTTPURLResponse?, error: Error?),file: StaticString = #file, line: UInt = #line) {
        ...
        sut.post(to: makeUrl(), with: makeValidData()) { receivedResult in
            ...
            case (.failure(let expetedError), .failure(let receivedError)): XCTAssertEqual(expetedError, receivedError, file: file, line: line)
            case(.success(let expetedData), .success(let receiveData)): XCTAssertEqual(expetedData, receiveData, file: file, line: line)
